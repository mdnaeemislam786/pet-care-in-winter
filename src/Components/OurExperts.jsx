    ...
    <div className=" bg-transparent py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600">Dedicated professionals ensuring your pet's winter wellness</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {experts.map((expert, index) => (
            <div key={index} className="bg-white rounded-2xl p-6 shadow-lg border border-cyan-200 text-center hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
              <div className="text-6xl mb-4">{expert.image}</div>
              <h3 className="text-xl font-bold text-cyan-700 mb-2">{expert.name}</h3>
              <p className="text-gray-600 mb-2">{expert.specialty}</p>
              <p className="text-cyan-600 font-semibold">{expert.experience} experience</p>
